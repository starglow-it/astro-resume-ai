import {
  validateEmail,
  validatePassword,
  generateScoreResumeItem,
} from "./customJs/custom.js";
...
function handleScore() {
  if (defaultResumes.length > 0) {
    for (const resume of defaultResumes) {
      ...
      const customCollectionItems = document.querySelectorAll(
        "#score-collection .custom-collection-item"
      );
      ...
      customCollectionItems.forEach((item) => {
        item.addEventListener("click", function () {
          ...
          // Save selectedResumeId into chrome extension storage so that it can be retrieved by auto bid extension.
          chrome.storage.local.set(
            {
              currentId: selectedResumeId,
            },
            () => {
              console.log("Profile Id selected: ", selectedResumeId);
            }
          );
          ...
          const selectUl = document.querySelector(
  ...
  if (isLoading) {
    ...
    aElements.forEach((a) => {
      ...
      sliderValue.style.color =
        score < 25 ? "red" : score >= 75 ? "#7df" : "gold";
...
function displayResumes(resumes) {
  ...
  if (Array.isArray(resumes) && resumes.length > 0) {
    ...
    selectElement.innerHTML =
      '<option value="" disabled selected>No resumes found</option>';
    defaultResumes = [];
    document.getElementById("score-preloader").style.display = "none";
    document.getElementById("no-item-text").innerHTML =
      'No resumes found. You need to create a resume first on the <a href="http://localhost:3000/add-profile/">main page</a>.';
  }
...
const handleLogInSuccess = async (isRemember = false, tokenParam = "") => {
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:8000/api/job_queries/${hostname}`
    );
    ...
    const jobData = await chrome.tabs.sendMessage(tabs[0].id, {
      action: "select_by_classname",
      className: jobContentQuery,
    });
    ...
    if (!!jobData.jobDescription) {
      ...
      const response = await fetch(
        "http://localhost:8000/api/resumes/cal_matching_scores/",
        {
          method: "POST",
          headers: {
  ...
  `;
      ...
      try {
        ...
        if (response.ok) {
          ...
          setTimeout(
            async () => await handleLogInSuccess({ tokenParam: key }),
            1500
          );
        } else {
...
//Navbar actions
document
  .getElementById("scan-job-nav-item")
...
document
  ...
  .addEventListener("click", async function (event) {
    ...
    if (isResumeGenerated) {
      ...
      document.getElementById("generate-resume-btn").innerText =
        "GENERATE RESUME";
...
document
  .getElementById("profile-nav-item")
  .addEventListener("click", function (event) {
    event.preventDefault();
    toggleScanJobBoard(false);
    toggleScoreBoard(false);
...
document
  .getElementById("sign-up-instead")
  ...
  chrome.tabs.query(
    ...
    async (tabs) => {
      ...
      chrome.storage.local.get([hostname], (result) => {
        if (result[hostname]) {
          const { titleSelector, descriptionSelector } = result[hostname];
...
document
  .getElementById("descriptionSelector")
...
document
  .getElementById("generate-resume-btn")
  .addEventListener("click", async function () {
    try {
      ...
      `;
        ...
        const response = await fetch(
          "http://localhost:8000/api/resumes/generate-resume/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          downloadBtn.innerText = `DOWNLOAD ( SCORE: ${Math.round(
            score * 100
          )} )`;
...
document
  .getElementById("scan-job-content")
  .addEventListener("click", async function () {
    if (isResumeGenerated) {
      document.getElementById("download-resume-btn").innerText = "DOWNLOAD";
      document.getElementById("download-resume-btn").disabled = true;
...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  let { isAuthenticated, token } = await chrome.storage.local.get([
    "isAuthenticated",
    "token",
  ]);
